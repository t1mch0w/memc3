...
static char* memc_get(memcached_st *memc, char *key, thread_param* p) {
  memcached_return_t rc;
  char *val;
  size_t len;
  uint32_t flag;
...
main(int argc, char **argv)
{
  ...
  for (t = 0; t < num_threads; t++) {
    ...
    tp[t].num_puts = tp[t].num_gets = tp[t].num_miss = tp[t].num_hits = tp[t].num_notfound = 0;
