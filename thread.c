...
enum store_item_type store_item(item *item, int comm, conn* c) {
    ...
    hv = hash(ITEM_key(item), item->nkey, 0);
	//printf("thread %ld start store_item for key %s and hv %d\n", syscall(SYS_gettid), ITEM_key(item), hv);
    item_lock(hv);
    ret = do_store_item(item, hv);
    item_unlock(hv);

	//printf("thread %ld finish store_item for key %s and hv %d\n", syscall(SYS_gettid), ITEM_key(item), hv);
    return ret;
}
...
enum store_item_type do_store_item(item *it, const uint32_t hv) {
	...
	//printf("thread %ld start do_store_item for key %s old_it %p new_it %p\n", syscall(SYS_gettid), key, (void*)old_it, (void*)it);
    if (old_it != NULL) {
        assert(old_it != it);
		...
		//printf("assert value = %d, thread_id = %ld, old_it = %p, key = %s, old_it->it_flags = %d, ITEM_LINKED = %d\n", (old_it->it_flags & ITEM_LINKED), syscall(SYS_gettid), (void*)old_it, key, old_it->it_flags, ITEM_LINKED);
	...
	//printf("thread %ld finish do_store_item for key %s old_it %p new_it %p\n", syscall(SYS_gettid), key, (void*)old_it, (void*)it);
    return  stored;
}
